...
def read():
    ...
    lst=[]
    term_dictionary = db['term_dictionary']	
    ctr = 0
    print "length d_title",
    print len(d_title)
    for key in d_title:
	...
	for kk in d_title[key]:
        ...
        print ctr
        db.term_dictionary.insert( { "Id":key ,"Tags": tags_d[key] , "Term_Body" : d_body[key] , "Term_Title" : d_title[key] })
	

    # We will insert into a mongo db
    print "term done"
    db.idf_dictionary_title.insert(idf_dictionary_title)
    db.idf_dictionary_body.insert(idf_dictionary_body)
